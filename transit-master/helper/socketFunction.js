...
module.exports = {
    ...
    check_socket_id: async function (connect_listener, socket_id) {
        ...
        console.log("2222222222222222222222222222222222222", connect_listener.userId);
        ...
        if (check_user) {
            console.log("iiiiiiiiiiiiiiiiiiiiiiiiffffffffffffffffffffffffffff");
    ...
    findGroupUsers: async function (get_data) {
        ...
        all_group_users = await group_users.findAll({
            attributes: ['id', 'unreadcount', 'userId', 'notification', [sequelize.literal('ifnull((SELECT socketId FROM socket_user WHERE userId = group_users.user_id LIMIT 1),"")'), 'socketId'],
                [sequelize.literal('ifnull((SELECT device_token FROM user WHERE id = group_users.user_id),"")'), 'device_token'],
                [sequelize.literal('ifnull((SELECT device_type FROM user WHERE id = group_users.user_id),"")'), 'device_type']
            ],
    ...
    get_block_status_users: async function (get_data) {
        ...
        get_user_block_status = await chatBlock.findOne({
            ...
            where: {
                [Op.or]: [{
                        userBy: get_data.senderId,
                        UserTo: get_data.receiverId
                    },
                    {
                        UserTo: get_data.senderId,
                        userBy: get_data.receiverId
                    }
    ...
    get_message: async function (get_msg_data) {
        ...
        if (get_user_status) {
            ...
            var get_message = await messages.findAll({
                attributes: ['id', 'createdAt', `type`, [sequelize.literal('(SELECT name FROM users WHERE users.id  = messages.senderId)'), 'SenderName'], 'message',
                    ...
                    [sequelize.literal('(SELECT image FROM users WHERE users.id  = messages.receiverId)'), 'ReceiverImage'], 'messageType'
                ],
    ...
    get_chat_listing: async function (get_chat_data) {
        var chat_data = await database.query(`select *,(select Count(*) from messages where (receiverId=${get_chat_data.userId} and senderId=${get_chat_data.userId})  and (readStatus=0)) as unreadcount  from (SELECT *,CASE WHEN senderId = ${get_chat_data.userId} THEN receiverId WHEN receiverId = ${get_chat_data.userId} THEN senderId  END AS user_idd,(SELECT message FROM messages where id=lastMessageId   and  deletedId!=${get_chat_data.userId})as lastMessage,(SELECT name FROM users where id=user_idd) as userName, ifnull((SELECT image FROM users where users.id=user_idd),'') as userImage,(SELECT  created  FROM messages where id=lastMessageId) as created_at ,(SELECT  messageType  FROM messages where id=lastMessageId ) as messageType ,(SELECT  chatConstantId  FROM messages where id=lastMessageId ) as chatConstantId
            ,(SELECT  type  FROM messages where id=lastMessageId  ) as Types,(SELECT  isOnline  FROM socketUser where userId=${get_chat_data.userId}) as isOnline from chatConstants where (senderId=${get_chat_data.userId} or receiverId=${get_chat_data.userId}))tt where deletedId!=${get_chat_data.userId}  and type = ${get_chat_data.type} order by lastMessageId desc`);
        return chat_data[0];
    },
        ...
        if (get_group_listing.length > 0) {
            ...
            if (groupIds.length > 0) {
                get_group_listing = await groups.findAll({
                    attributes: ['id', 'name', 'image', 'createdAt', [sequelize.literal('(SELECT COUNT(id) FROM group_users WHERE group_id = groups.id)'), 'totalUsers'],
                        [sequelize.literal(`(SELECT unreadcount FROM group_users WHERE group_id = groups.id AND user_id = ${get_data.userId})`), 'unreadcount'],
                        [sequelize.literal('ifnull((SELECT message FROM messages WHERE groupId = groups.id order by id desc limit 1),"")'), 'lastMessage'],
                        [sequelize.literal('ifnull((SELECT created FROM messages WHERE groupId = groups.id order by id desc limit 1),groups.created_at)'), 'lastMessageCreated'],
                        [sequelize.literal('ifnull((SELECT messageType FROM messages WHERE groupId = groups.id order by id desc limit 1),0)'), 'messageType'],
                        [sequelize.literal('(SELECT user_id FROM group_users WHERE group_id = groups.id and is_admin = 1)'), 'adminId'],
                        [sequelize.literal('(SELECT count(id) from messages where groupId = groups.id)'), 'groupMessagesCount'],
                        [sequelize.literal('(SELECT count(id) from group_messages_delete where groupId = groups.id and userId = ' + get_data.userId + ')'), 'deleteMessagesCount']
                    ],
                    where: {
                        id: {
                            [Op.in]: groupIds
                    ...
                    order: [
                        [sequelize.col('lastMessageCreated'), 'DESC']
                    ]
                });
    ...
    delete_chat_users: async function (get_delete_data) {
        get_delete_status = await messages.findAll({
            where: {
                deletedId: 0,
                [Op.or]: [{
                        senderId: get_delete_data.userId,
                        receiverId: get_delete_data.user2Id
                    },
                    {
                        receiverId: get_delete_data.userId,
                        senderId: get_delete_data.user2Id
                    },
                ]
            }
        });
        ...
        get_block_status_data = await blockedUser.findOne({
            where: {
                ...
                [Op.or]: [{
                    ...
                    {
                        user2Id: get_block_status.userId,
                        userId: get_block_status.user2Id
                    }
    ...
    delete_chat_list_data: async function (get_data) {
        ...
        get_block_status_data = await chatConstants.findOne({
            where: {
                ...
                [Op.or]: [{
                        senderId: get_data.userId,
                        receiverId: get_data.user2Id
                    },
                    {
                        receiverId: get_data.userId,
    ...
    image_base_64: async function (get_messagetype, get_message, extension_data) {
        ...
        var base64Str = data;
        var upload_path = path.join(__dirname, '../uploads/image/' + filename);
